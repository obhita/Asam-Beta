...
namespace WordDocumentGenerator.Library
{
    ...
    public class OpenXmlHelper
    {
        ...
        private int _hypercount = 0;

        public void MakeContentHyperlink(SdtElement element, string location)
        {
            var content = GetSdtContentOfContentControl(element);
            element.RemoveChild(content);
            var hyperlink = new Hyperlink(content) { Anchor = location };
            var stdRun = new SdtRun { SdtContentRun = new SdtContentRun(hyperlink) };
            SetSdtContentKeepingPermissionElements(element, stdRun);
        }
        ...
        private void AddToListIfCompositeElement(IList<OpenXmlCompositeElement> elementsList, OpenXmlElement newElement)
        {
            ...
            if(newElement is Hyperlink || newElement is BookmarkStart || newElement is BookmarkEnd)
            {
                return;
            }
