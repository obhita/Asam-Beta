(function ($) {
    ...
    $.fixSizes = function () {
        ...
        if (activeContent.height() + $.getTotalHeaderHeight(navigation[0])
            > $('#rootTabs #tabContent').height() - 15) {
            navigation.accordion("option", "heightStyle", "fill");
        } else {
            navigation.accordion("option", "heightStyle", "content");
        }
    ...
    $.getFormUrl = function() {
        ...
        return window.asam.urlformat.replace("{section}", section).replace("/{subsection}", subSection);
    };

    $.initializing = function (active, urlformat) {
        window.asam = { urlformat: urlformat };
        window.assessmentFormDirty = false;
        var navigation = $('#assessmentNavigation');
    ...
    $.navigateErrors = function () {
        ...
        form.find('div.yesno-radiobuttons :radio').each(function () {
            var self = $(this);
            var divWrapper = self.closest('.yesno-radiobuttons');
            for (var i = 0, attrs = this.attributes, l = attrs.length; i < l; i++) {
                if (attrs[i].name.indexOf("data-val-completeness") == 0) {
                    divWrapper.attr(attrs[i].name, attrs[i].value);
                    self.removeAttr(attrs[i].name);
                    i--;
                    l--;
                }
            }
            self.parent().on("focusin focusout",':focusable',
                $.proxy(function (event) {
                    $(this).trigger(event.type);
                }, this));
        });
